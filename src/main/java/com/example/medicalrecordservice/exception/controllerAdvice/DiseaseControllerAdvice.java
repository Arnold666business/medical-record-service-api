...
public class DiseaseControllerAdvice {
    ...
    public ResponseEntity<DiseaseCodeInfo> diseaseRecurrenceHandler(ActiveDiseaseExistsException exception){
        ...
        return ResponseEntity.status(HttpStatus.CONFLICT).body(DiseaseCodeInfo.builder()
                .message(String
                        .format("Patient with ID %s already has a disease with code %s"
                                ,patientId, mkb10Code))
                .mkb10Code(mkb10Code).patientId(patientId).build());
    }
    ...
    public ResponseEntity<DiseaseErrorInfo> notFoundDiseaseHandler(NotFoundDiseaseException exception){
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(DiseaseErrorInfo.builder()
                .id(diseaseId)
                .message(String.format("Patient with ID %s, diseases with ID %s not found",
                        patientId, diseaseId)).build());
    }

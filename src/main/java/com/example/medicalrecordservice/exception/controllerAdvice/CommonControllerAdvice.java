...
public class CommonControllerAdvice {
    ...
    public ResponseEntity<ResponseError> notFoundPatientHandler(NotFoundPatientException exception){
        ResponseError response = ResponseError.builder()
                .message(String.format("Patient with ID %s not found", exception.getPatientId())).build();
    ...
    public ResponseEntity<ValidationErrorResponse> constraintViolationExceptionHandler(ConstraintViolationException exception){
        final List<Violation> violationList = exception.getConstraintViolations().stream()
                .map(violation -> Violation.builder().message(violation.getMessage())
                        .fieldName(violation.getPropertyPath().toString()).build()).toList();
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ValidationErrorResponse.builder()
                .violations(violationList).build());
    }
    ...
    public ResponseEntity<ValidationErrorResponse> methodArgumentNotValidExceptionHandler(MethodArgumentNotValidException exception){
        final List<Violation> violationList = exception.getBindingResult().getFieldErrors().stream()
                .map(error -> Violation.builder().message(error.getDefaultMessage())
                        .fieldName(error.getField()).build()).toList();
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(ValidationErrorResponse.builder()
                .violations(violationList).build());
    }
    ...
    public ResponseEntity<ResponseError> httpRequestMethodNotSupportedExceptionHandler(HttpRequestMethodNotSupportedException exception){
        ...
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(ResponseError.builder().message(message)
   ...
   @ExceptionHandler(NoHandlerFoundException.class)
    public ResponseEntity<ResponseError> noHandlerFoundExceptionHandler(NoHandlerFoundException exception){
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseError.builder().message(
                "Resource not found: " + exception.getRequestURL()
    ...
    public ResponseEntity<ResponseError>  uuidExceptionHandler(MethodArgumentTypeMismatchException e){
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ResponseError.builder()
                .errors(List.of(e.getMessage())).message(message).build());
    }
    ...
    public ResponseEntity<ResponseError> missingPathVariableExceptionHandler(MissingPathVariableException exception){
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ResponseError.builder()
                .message(message).errors(List
                        .of("There may be extra literals or spaces, or missing characters in the query.")).build());
    }
